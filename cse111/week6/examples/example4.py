...
#A Python lambda function is a small anonymous function, 
# meaning a small function without a name. 
# A lambda function is always a small function because the Python language 
# restricts a lambda function to just one expression. 
...
def main():
    ...
    #They both yield same result.
    
    
    #Notice in the below example that the lambda function is defined 
    #inside the parentheses for the call to the map function.
    
    # Convert each Fahrenheit temperature to Celsius and store
    # the Celsius temperatures in a list named cels_temps.
    cels_temps2 = list(map(lambda fahr: round((fahr - 32) * 5 / 9, 1), fahr_temps))

    

    # Print the Celsius temperatures.
    print(f"Celsius: {cels_temps}")
    print(f"Celsius: {cels_temps2}")
